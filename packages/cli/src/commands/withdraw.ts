...
export function registerWithdraw(program: Command): void {
  program
    .command("withdraw")
    .description("Prepare a withdraw (redeem) transaction")
    .requiredOption("--asset <symbol>", "Underlying asset to withdraw")
    ...
    .action(async (opts) => {
      await executeLendCommand("withdraw", opts, program);
    });
}
